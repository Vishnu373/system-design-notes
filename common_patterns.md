...
- Indexing — pre-computed lookup structures on the DB so queries find data faster without scanning every row. (Just like the index of book)
...
- Batching — group multiple writes together and send them in one operation instead of one-by-one. Reduces overhead (like memtable in LSM trees).
- Asynchronous writes — return success to the client immediately, persist to DB in the background. Lower latency but risk of data loss on crash.

Case 3: Real-time data
- WebSockets — full-duplex, persistent connection between client and server. Both can send messages anytime. Best for chat, live collaboration.
- Server-Sent Events (SSE) — server pushes updates to client over a single long-lived HTTP connection. One direction only (server → client). Best for live feeds, streaming responses.
...
- Idempotency — designing operations so retrying them multiple times produces the same result as doing it once. Prevents duplicate charges, duplicate records, etc.
